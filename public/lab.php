...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .error-card .wolfram-card-header { color: #ff6b6b; border-color: rgba(255,107,107,0.3); }
        ...
        /* Input preview */
        .wolfram-input-wrap { flex:1; display:flex; flex-direction:column; gap:6px; }
        .wolfram-input-wrap .wolfram-input { color:var(--text); caret-color:var(--cyan); width:100%; }
        .wolfram-input-wrap .wolfram-input::placeholder { color:var(--muted); }
        .wolfram-latex-preview { display:none; padding:8px 14px; background:rgba(0,229,255,0.06); border:1px solid rgba(0,229,255,0.2); border-radius:8px; font-size:15px; color:var(--text); min-height:36px; align-items:center; overflow-x:auto; }
        .wolfram-latex-preview.visible { display:flex; }
        .wolfram-latex-preview .katex { font-size:1.15em; }
        .wolfram-latex-preview .katex-display { margin:0; }
...
<body>
    ...
    <div class="app-layout">
        ...
        <div id="wolframSection" class="wolfram-section hidden" style="display:none;">
            <div class="wolfram-header">
                ...
                <div class="wolfram-input-row">
                    <div class="wolfram-input-wrap">
                        <input type="text" class="wolfram-input" id="wolframInput"
                            placeholder="Escribe tu problema... ej: derivada x^2+3x, resolver 2x+5=11, integral sen(x)dx"
                            autocomplete="off" spellcheck="false"
                            onkeydown="if(event.key==='Enter') solveWolfram()"
                            oninput="updateWolframPreview()">
                        <div class="wolfram-latex-preview" id="wolframLatexPreview"></div>
                    </div>
                    <button class="wolfram-btn" onclick="solveWolfram()">Resolver</button>
                </div>
    ...
    function updateWolframPreview() {
        ...
        var preview = document.getElementById('wolframLatexPreview');
        if (!preview) return;
        ...
        // Detect if there are LaTeX/math symbols worth rendering
        var hasLatex = /\\[a-zA-Z]|[\^_{}]|[∫∑∏√π∞∂∇±≈≠→∀∃∩∪]/.test(val);
        // Also try to render plain math expressions
        var hasMath = /[+\-*/^()=]/.test(val) && /\d|[a-zA-Z]/.test(val);
        ...
        if (hasLatex) {
            try {
                preview.innerHTML = '';
                katex.render(val, preview, { throwOnError: true, displayMode: false, strict: false, trust: true });
                preview.classList.add('visible');
                rendered = true;
            } catch(e) {}
        }

        // 2) Try math.js parse → toTex → KaTeX
        if (!rendered) {
            try {
                var node = math.parse(val);
                var tex = node.toTex({ parenthesis: 'keep' });
                preview.innerHTML = '';
                katex.render(tex, preview, { throwOnError: true, displayMode: false, strict: false });
                preview.classList.add('visible');
                rendered = true;
            } catch(e) {}
        }

        // 3) Fallback: show raw text in preview
        if (!rendered) {
            preview.classList.add('visible');
            preview.textContent = val;
        }
