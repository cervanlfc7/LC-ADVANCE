...
(function () {
  ...
  document.addEventListener("click", function (e) {
    ...
    if (btn) {
      ...
      const feedbackMsg =
        btn.dataset.feedback || (isCorrect ? "✅ Correcto" : "❌ Incorrecto");
      ...
      if (container.querySelector("#pollutionQuizFeedback")) {
        window.pollutionQuizScore =
          (window.pollutionQuizScore || 0) + (isCorrect ? 1 : 0);
        const answered =
          container.querySelectorAll(".quiz-option:disabled").length /
          (q ? q.querySelectorAll(".quiz-option").length : 1);
  ...
  document.addEventListener("keydown", function (e) {
    if (e.key === "Enter" || e.key === " ") {
      ...
      if (
        active &&
        active.classList &&
        active.classList.contains("quiz-option")
      ) {
        e.preventDefault();
        active.click();
      }
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Iniciar ranking en dashboard solo si hay sesión activa
  const path = window.location.pathname;
  const href = window.location.href;
  const isPublicPage = path.includes('login.php') || 
                      path.includes('register.php') || 
                      path.includes('guest_login.php') ||
                      path.includes('quiz.php') ||
                      href.includes('login.php') ||
                      href.includes('register.php');
...
// DESHABILITADO: Botón hamburguesa no necesario para dashboard moderno
// document.addEventListener('DOMContentLoaded', function(){
//     try {
...
function fetchAndUpdateDashboard() {
  fetch("api/ranking.php", {
    ...
    .then((data) => {
      ...
      if (puntosNecesariosEl)
        puntosNecesariosEl.textContent = puntos_necesarios;

      // Actualizar barra de progreso
      const puntos_base = (data.nivel || 1) * 500;
      const progreso = Math.min(
        100,
        Math.max(0, (((data.puntos || 0) - puntos_base) / 500) * 100),
      );
      ...
      if (badgesContainer && data.badges) {
        if (data.badges.length === 0) {
          ...
          badgesContainer.innerHTML = data.badges
            .map(
              (b) =>
                `<span class="badge ${b.tipo || "bronze"}">${b.nombre}</span>`,
            )
            .join("");
        }
      }
      ...
      if (rankingBody && data.ranking) {
        if (data.ranking.length === 0) {
          rankingBody.innerHTML =
            '<tr><td colspan="3" class="text-muted">Sé el primero en el ranking 🏆</td></tr>';
        } else {
          rankingBody.innerHTML = data.ranking
            ...
            .join("");
          console.log(
            "Ranking actualizado con " + data.ranking.length + " jugadores",
          );
        }
      } else {
