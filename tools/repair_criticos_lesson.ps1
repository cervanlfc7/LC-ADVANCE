# Reparación definitiva y correcta de la lección Puntos Críticos en src/content.php
$file = 'c:\xampp\htdocs\LC-ADVANCE\src\content.php'
$contentRaw = [System.IO.File]::ReadAllText($file, [System.Text.Encoding]::UTF8)

# Buscamos el inicio y fin del bloque a reemplazar
# Inicio: después del titulo de la lección puntos-criticos-maximos-minimos
# Fin: antes del comentario de Ecosistemas que sigue

# Vamos a buscar por strings exactos para ser precisos
$startMarker = "'titulo'  => 'Puntos Críticos: Máximos y Mínimos Locales – Prueba de la Primera Derivada',"
$endMarker = "/* =====================================" # Inicio del quiz ampliado
...
# El nuevo bloque completo con la sintaxis PHP correcta
$newContent = @"
...
<div class="leccion-matematicas-criticos">
    ...
    <script>
        ...
        document.querySelectorAll(".btn-solucion").forEach(b => {
             b.onclick = () => toggleSol(b.getAttribute("data-target"), b);
        });
        ...
        const evalBtn = document.getElementById("evalCritQuiz");
        if(evalBtn) {
            evalBtn.onclick = () => {
                ...
                for(let k in ans) {
                    ...
                    let labels = document.querySelectorAll(`input[name="${k}"]`);
                    labels.forEach(r => {
                    ...
                    html += `<li>Pregunta ${k.slice(-1)}: <span style="color:${isOk?'#39ff14':'#ff3366'}">${isOk?'¡Correcto!':'Incorrecto'}</span><br><small>${explanations[k]}</small></li>`;
                }
...
$combined = $before + $newContent + $after
[System.IO.File]::WriteAllText($file, $combined, [System.Text.Encoding]::UTF8)
Write-Host "Lección Puntos Críticos REPARADA exitosamente."
