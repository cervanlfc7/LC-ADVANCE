# Reemplazo de la lección Puntos Críticos en src/content.php
$file = 'c:\xampp\htdocs\LC-ADVANCE\src\content.php'
$lines = Get-Content $file -Encoding UTF8
Write-Host ("Total lines before: " + $lines.Count)

# Rango de la lección (1-indexed 664 a 1294) -> (0-indexed 663 a 1293)
$before = $lines[0..662]
$after  = $lines[1294..($lines.Count - 1)]
    ...
    <section>
        ...
        <div class="concept-grid">
            ...
            <div class="concept-card">
                ...
                <ul>
                    <li>$f'(x) > 0 \Rightarrow$ La función sube.</li>
                    <li>$f'(x) < 0 \Rightarrow$ La función baja.</li>
                </ul>
            </div>
        </div>
        ...
        <table class="tabla-signos">
            <thead>
                <tr>
                    <th>Cambio de Signo</th>
                    <th>Comportamiento en la Gráfica</th>
            ...
            <tbody>
                <tr class="max">
                    <td>(+) a (-)</td>
                    <td>Sube $\nearrow$ y luego baja $\searrow$</td>
                    <td>MÁXIMO LOCAL</td>
                </tr>
                <tr class="min">
                    <td>(-) a (+)</td>
                    <td>Baja $\searrow$ y luego sube $\nearrow$</td>
                    <td>MÍNIMO LOCAL</td>
                </tr>
                <tr>
                    <td>Sin cambio</td>
                    <td>Mismo sentido (sube-sube o baja-baja)</td>
                    <td>PUNTO DE INFLEXIÓN / NADA</td>
                </tr>
            </tbody>
        </table>
    </section>
    ...
    <section>
        ...
        <div class="concept-grid">
            ...
            <div class="concept-card" style="border-left-color: var(--neon-min);">
                ...
                <p>En $x=0$, $f'(0)$ no existe (es un pico), pero claramente hay un **mínimo**. El criterio funciona igual analizando signos.</p>
            </div>
        </div>
    </section>
    ...
    <section>
        ...
        <div class="recursos">
            ...
            <ul>
                ...
                <li><a href="#">Video: Optimización en la vida diaria</a></li>
            </ul>
        </div>
    </section>
    ...
    <script>
        (function() {
            ...
            function draw() {
                ...
                // Encontrar críticos (simple scan)
                let crits = [];
                for(let x=xmin+0.2; x<xmax-0.2; x+=0.05) {
                    ...
                    } else if(Math.abs(d1) < 0.05 && Math.abs(d2) < 0.05 && Math.abs(d1-d2) < 0.01) {
                         // Posible inflexión si la derivada es muy baja pero no cambia signo
                         // Solo detectamos si está cerca de 0
                ...
                crits.forEach(c => {
                    ...
                    let color = c.type==='max' ? 'var(--neon-max)' : (c.type==='min' ? 'var(--neon-min)' : 'var(--neon-crit)');
                ...
                svg.innerHTML = `
                    ...
                    <path d="${d}" fill="none" stroke="var(--neon-curva)" stroke-width="3" stroke-linecap="round" />
                    ${pointsHTML}
                `;
                labelsList.innerHTML = listHTML;
            }
            ...
            // Toggle soluciones y errores
            document.querySelectorAll(".btn-solucion").forEach(b => {
                b.addEventListener("click", () => {
                    let target = document.getElementById(b.getAttribute("data-target"));
                    target.classList.toggle("visible");
                    b.textContent = target.classList.contains("visible") ? "OCULTAR RESOLUCIÓN" : "VER RESOLUCIÓN";
                });
            });

            document.querySelectorAll(".error-card").forEach(c => {
                c.addEventListener("click", () => c.classList.toggle("expandido"));
            });

            // Quiz logic
            document.getElementById("evalCritQuiz").addEventListener("click", () => {
                ...
                let score = 0, html = "<h4>📊 Resultados</h4><ul>";
                for(let k in ans) {
                    let sel = document.querySelector(`input[name="${k}"]:checked`);
                    let isOk = sel && sel.value === ans[k];
                    if(isOk) score++;
                    html += `<li>Pregunta ${k.slice(-1)}: <span style="color:${isOk?'var(--neon-min)':'var(--neon-alerta)'}">${isOk?'¡Correcto!':'Incorrecto'}</span><br><small>${explanations[k]}</small></li>`;
                }
                html += `</ul><p>Puntaje: ${score}/3</p>`;
                const feed = document.getElementById("quizCritFeedback");
                feed.innerHTML = html; feed.style.display = "block";
            });
...
$newLines = $before + ($newHTML -split "`n") + $after
Write-Host ("Total lines after: " + $newLines.Count)

$newContent = $newLines -join "`n"
[System.IO.File]::WriteAllText($file, $newContent, [System.Text.Encoding]::UTF8)
Write-Host "Lección Puntos Críticos actualizada exitosamente."
